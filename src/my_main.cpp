...
cv::VideoWriter original_video("original.mp4", cv::VideoWriter::fourcc('m', 'p', '4', 'v'), 30, cv::Size(640,360));
//cv::VideoWriter sgmt_video("segmentation.mp4", cv::VideoWriter::fourcc('m', 'p', '4', 'v'), 30, cv::Size(640,360));
    ...
    TensorRTEngine& engine)
{
    ...
    //Step 2-1: test를 위한 영상 저장
    original_video << frame;

    // Step 3: 각 프레임에 대해 추론 수행
    cv::Mat mask;
    cudaStream_t stream;
    cudaStreamCreate(&stream); // CUDA 스트림 생성 (비동기 모드일 때)
    engine.runInference(frame, mask, stream); // 비동기 추론 수행 추가
    //engine.runInference(frame, mask);
    if (mask.empty()) {
    ...
    // Step 4: GPU 작업이 완료될 때까지 동기화 (추론이 끝났을 때), (비동기 모드일 때)
    cudaStreamSynchronize(stream);

    // Step 5: "gray_segmentation_image/compressed" TOPIC으로 Publishing
    std_msgs::msg::Header hdr;
    //sensor_msgs::msg::CompressedImage::SharedPtr img;
    //auto img = cv_bridge::CvImage(hdr, "mono8", frame).toCompressedImageMsg(); //test(라인트레이서 할 때 필요한 원본 이미지)
